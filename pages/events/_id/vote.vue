          ...
          v-expansion-panels
            v-expansion-panel(v-for="(entry, i) in entries" :key="entry.id")
              ...
              v-expansion-panel-content.text--secondary
                ...
                div(v-if="(entry.videoType === 'online' || entry.videoType === 'mixed')" color='blue')
                ...
                //-   span(v-if="isVoted('オンラインブルーグラスアワード')")
                //-     v-btn.ma-4(color="red" small v-if="availableVote && $firebase.currentUser" @click="vote(entry, 'オンラインブルーグラスアワード')" block)
                //-       | {{voteString('オンラインブルーグラスアワード')}}
                //-   span(v-if="!isVoted('オンラインブルーグラスアワード')")
                //-     v-btn.ma-4(color="indigo" small v-if="availableVote && $firebase.currentUser" @click="vote(entry, 'オンラインブルーグラスアワード')" block) 
                //-       | オンラインブルーグラスアワード
                
                //- span(v-if="(entry.videoType != 'live')" v-for="(voteKind, i) in prizes" :key='voteKind')
                //- span(v-for="(voteKind, i) in prizes" :key='voteKind')
                //-   v-btn.ma-4(:color="voteColor(voteKind)" small v-if="availableVote && $firebase.currentUser" @click="vote(entry, voteKind)" block )
                //-     | {{ voteString(voteKind) }}
</template>
  ...
  methods: {
    voteString(kind) {
      if (this.isVoted(kind)) {
        ...
        return (
          '投票済み: ' + targetEntry.name + 'への ' + kind + ' 投票を取り消す'
        )
      }
      return kind
    },
